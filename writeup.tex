...
In Figure \ref{fig:avg_value} we plot the average and median value of firms' exporting experience during the 25 periods immediately before the shock and the 25 periods immediately thereafter.\footnote{%
...
As the productivity distribution is right-skewed, it is not surprising that the means are significantly higher than the medians. Before the exchange rate shock, the mean is around 1.7 million dollars, while the median is only 80,000
dollars. After the shock, the values ultimately converge to around 20 percent higher or lower than before the shock, but there is some overshooting in each case. For the positive shock, this pattern reflects the role of matching frictions, which delay the entry of marginally profitable firms. For the negative shock, the pattern reflects the role of fixed costs and relationship-specific shocks ($y_{ijt}$), which cause the eventual termination of matches at some marginally profitable firms. 
