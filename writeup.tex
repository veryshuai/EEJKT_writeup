...
The sensitivity matrix (Appendix C) implies that equation ($ii$) helps most to identify the fixed costs of maintaining an established foreign match, $F^{f}$. That is, conditioning on sales, $X_{ijt}^{f},$ matches are more likely to survive when fixed costs are low. It also proves to be helpful in identifying the jump size, $\Delta_{y}$, since failure rates are also affected by the volatility of $z_{ijt}$. 
...
{tab:haz_reg} concern the distribution of success rates, $\theta $.
...
regression line must be zero.\medskip} We choose this functional form because
