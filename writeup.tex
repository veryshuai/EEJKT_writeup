...
Like firms' exporting stints (Table \ref{tab:Brooks}), most of their buyer-seller matches are short-lived. Even among those involving more than one shipment, the overall year-to-year match death rate is roughly 40 percent, as we will show later. Not surprisingly, therefore, there is a great deal of flux in exporters' portfolio of clients.
...
Table \ref{tab:trans_probs} reports the probability with which a Colombian exporter transitions to a different number of relationships the following year.A firm that stops exporting but re-appears as an exporter sometime later in our sample period is considered to have gone "dormant", while those exporters that drop to zero foreign sales for the extent of our sample are considered to have gone "out" of exporting. Those that have never been observed to export constitute the pool of potential entrants.

Among first-time exporters, 93.2 percent sell to only one firm.\footnote{Many of these matches involve a single shipment. As we will show later, the overall match success rate (i.e., multi-shipment rate) is roughly 41 percent.} Of these, 61.9 percent don't export the next year, and only about 6 percent go on to establish a larger number of relationships. For firms with 3 relationships in a year, about 12 percent enter into a larger number of relationships the next year. Hence, in addition to an enormous amount of churning among smaller exporters, we see a general tendency to lose clients on net from one year to the next.
...
\begin{subfigure}[b]{0.45\textwidth}
    ...
    \end{subfigure}
%\caption{Baseline responses to permanent shock: Market level outcomes}
...
\begin{subfigure}[b]{0.45\textwidth}
        ...
        \caption{Total active exporters: Revaluation}
    \end{subfigure}
%\caption{Baseline responses to permanent shock: Composition}
...
\begin{subfigure}[b]{0.45\textwidth}
        ...
        \caption{Total active exporters: Revaluation}
    \end{subfigure}
%\caption{Compositional effects of a permanent shock: baseline vs. other specifications}
