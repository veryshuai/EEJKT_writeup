...
Despite rapid match turnover, adjustments to a new exchange rate regime take time to play out. During the first post-shock year, valuation effects account for almost all of the movement in aggregate exports. That is, the dominant effect of a 20 percent peso depreciation is to increase the peso-denominated revenue flow from existing clients by 20 percent (Panel a), and the immediate effect of a 20 pecent peso appreciation is to decrease these revenues by 20 percent (Panel b). However, as time progresses the total effect of the exchange rate shock grows, both because the total number of exporters adjusts, and because the number of matches per exporter move sympathetically (refer to panels c through f). These processes take place over a period of roughly 15 years, adding an extra 40-50 percent to the initial valuation response.  \textbf{[JT: shall we include more precise figures?]}
...
During the transition period, all margins of adjustment move in sympathy. In fact, if we were only interested in the rate at which new exporters displace incumbent exporters, or the rate at which new matches displace incumbent matches, it would matter very little whether we were analyzing the aftermath of a permanant devaluation, a permanent revaluation, or a period without any regime swithing. This is apparent from Figure \ref%
{fig:export_dynamics_average}, which plots the percentage contribution of each type of match to export aggregates. Note that the post shock transition patterns in all figures coincide very closely with the no-shock (thin line) patterns. 

\subsubsection{Learning, networks and aggregate export dynamics}
Nonetheless, the relative contributions of the different types of matches \emph{do} depend upon the learning effects and visibility effects we have embedded in our model. To demonstrate how, we next contrast the post-shock evolution pattern implied by our benchmark model (column 1 of Table \ref{tab:struct_param}) with the pattern implied by the known-$\theta ^{f}$ version of the model (column 3 of Table \ref{tab:struct_param}), normalizing predicted values of aggregate sales, matches and exporters to unity in the initial shock period. And we contrast the dynamic implications of the known-$\theta ^{f}$ version of the model with those of a model in which $\theta ^{f}$'s are known \emph{and} all firms have maximum visibility, regardless of their exporting history. \footnote{%
...
Figure \ref{fig:export_dynamics_average_alt} depicts the simulated responses of total sales, total matches, and number of exporters to a 20 percent exchange rate appreciation (left-hand panels) and a 20 percent exchange rate depeciation (right-hand panels). As in figure \ref{fig:export_dynamics_average}, each panel distinguishes the contributions of icumbent matches (yellow region), new matches with incumbent exporters (orange region) and new matches with new exporters (blue region). But in this figure, we have superimposed lines that show how the shaded areas would have shifted if firms knew their $\theta ^{f}$ draws with certainty (dashed red lines), and if they not only knew their $\theta ^{f}$ values, but also had "full" visibility (dashed green lines).


Several clear messages emerge from this exercise. First, as with Figure \ref{fig:export_dynamics_average}, none of the patterns in Figure \ref{fig:export_dynamics_average_alt} depends qualitatively upon whether we are considering a permanent devaluation or a permanent revaluation--the rates at which the different types of matches change market shares are general features of the new cohort maturation processes. Quantitatively, however, giving all firms total visibility and/or perfect knowledge of their products' appeal increases new firms' share of sales relatively more when the shock is negative. The dominant effect appears to be that, with more selection on the entry margin, fewer new firms are close to the exit threshold. \textbf{[JT: how can we confirm this?]}

Second, regardless of whether visibility and/or knowledge of $\theta^{f}$ improve with experience, the matches of entering cohorts displace the matches that existed at the time of the shock at almost exactly the same pace. That is, the border between the orange and yellow regions nearly coincides with the dashed red and green lines in all panels. 

Third, however, when firms without experience know their true $\theta^{f}$ values, the share of new matches that goes to new exporters is substantially higher. Why? When potential exporters know their product appeal, entering cohorts are dominanted by firms with high $\theta^{f}$'s, and these firms invest relatively heavily in search. Firms that were incumbent in period 0 also tend to have high $\theta^{f}$'s and invest heavily in search, but since many of these firms were already well established, revealing their $\theta^{f}$ draws to them doesn't change their behavior as much.

Finally, when firms always know their $\theta^{f}$ draws, full visibility matters a bit, especially when predicting the shares of the different types of matches. Full visibility reduces marginal search cost and this encourages relatively more matching among high productivity (high  $\varphi$) firms, which tend to be common in cohorts that have survived a shakedown period. Accordingly, this effect becomes more important over time as the share of post-shock entrants in the population of exporters grows (refer to panels e and f). Nonetheless, just as we found in Section \ref{sec:value_of_rel} above, visibility effects are not as important as learning effects. 
