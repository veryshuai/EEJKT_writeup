...
records regarding exporter maturation, transitions across states, and the
...
of learning, the role of luck in market penetration, and the key drivers of aggregate export dynamics. Learning and visibility effects both prove to be statistically significant, but the former are more important than the latter, and while they both affect the maturation processes of new exporters, neither matters as much as search frictions in shaping aggregate export dynamics.
\end{abstract}
...
customs records were expressed.\medskip} We now briefly describe these regressions
...
{FOC_MS_learning}). Because this is a central object in our model, there is no simple mapping from its coefficients to particular elements of $\Lambda$. Nonetheless, as we discuss below, the sensitivity matrix developed in Andrews et al. (2017) suggests that this equation is most helpful in identifying the fixed costs of maintaining a relationship, $F^{h}$ and $F^{f}$, the parameters of the $\theta$ distribution, $\alpha$ and $\beta$, and the network/congestion parameter, $\gamma$. 
...
search intensity is not very sensitive to success rates, but it strongly increases
...
Only active matches are included in the sample.\medskip} Our model implies that
...
unsuccessful meetings that did not lead to business relationships.\medskip}
...
governed by the jump size, $\Delta_{y}.$
