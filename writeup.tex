...
%TCIDATA{LastRevised=Saturday, June 13, 2020 10:54:20}
...
elsewhere. Our model resembles theirs in that exporters learn about their
products' through early shipments, and when new exporters survive, their
sales tend to grow rapidly. Aeberhardt et al. (2014)\ and Araujo et al.
(2016) explain the small scale and high exit rate of new exporters (inter
alia) by assuming that exporters are initially uncertain about the
reliability of their new buyers, whom they find through a random matching
process. The learning mechanisms built into these models are different from
ours, but they too are designed to capture some of the same patterns that we
target. \textbf{[JT: this paragraph was based on superficial review of
papers--I need to confirm that these descriptions are accurate.]}
...
market-wide shocks.  Specifically, we use a series of randomly-generated aggregate rate shocks to simulate the model, and then simulate the model again with the same shocks, but increasing their values by 20\% after a particular date.  The shock progression is graphed in Figure \ref{fig:exch_shk_cmp} Panel \ref{fig:raw_exch_rate}, with the shock arriving at time zero.  As with the rest of the series in this section, in the figure the shocks are normalized to one at the time of impact.  In order to show how the model interacts with these shocks, in Panel \ref{fig:total_matches_exch_rate} we graph the total number of matches across all firms in our baseline model along with the baseline exchange rate series.  Changes in total matches are visibly correlated with exchange rate changes, but the total match series is less volatile than the exchange rate.

 \begin{figure}
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/exch_rate_series_compare}
        \caption{Baseline vs devaluation}
        \label{fig:raw_exch_rate}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/total_matches_exch_rate_series}
        \caption{Total matches vs baseline rate}
        \label{fig:total_matches_exch_rate}
    \end{subfigure}
        \caption{Series of Peso to Dollar Exchange Rate, 20\% Peso Devaluation}
...
numbers of matches, which are depicted in Panels (c) and (d) of Figure \ref%
...
series without learning.  Both series see matches adjust slowly to the devaluation,
with more than five years between the shock and the maximum difference between the baseline series and 
the devaluation series.  In the learning version of the model, there is 20\% growth in matches seven years 
after the devaluation, while in the no-learning series there is only 15\% growth.  After 15 years, however,
both series exhibit only 15\% growth relative to the baseline series.

Panels (e) and (f) display a similar pattern for firm entry.  Seven years after the devaluation there are far 15\% more exporting firms relative to the baseline in the learning version of the model, but only 5\% more firms in the no-learning version.  After 15 years, the relative growth of exporters in the two model versions is comparable.  We interpret this pattern as showing how in the version of the model with learning, exporters with little experience enter the market due to the devaluation, learn after a few years that they have low success probability, and then exit.

In sum, search frictions themselves cause quite sluggish adjustment to shocks, with full adjustment only after more than five years.  Learning increases the volatility of bilateral export flows in the medium run.  In our experiment, exporters are induced by a favorable exchange rate shock to enter the market and learn about their success probabilities.  This experimentation has little effect on the market aggregates relative to the no-learning model after fifteen years.
