...
The shaded areas in these figures break down the total value of an export aggregate---sales, matches, or exporters---into contributions from matches that existed in period 0 (yellow), new (post-shock) matches with exporters existing in period 0 (orange), and new (post-shock) matches with exporters that entered after period 0 (blue). Each panel also includes a set of thin lines that show how the boundaries between the shaded areas would have been different if there had been no permanent shock.%
\footnote{% 
