...
%TCIDATA{LastRevised=Tuesday, June 02, 2020 11:20:37}
...
\\
{\small (first draft: February, 2014)} }
...
this paragraph has not really been updated.]}
...
be those that result in at least two shipments. These assumptions are
consistent with the large number of one-off shipments we observe between
...
vary across different types of exports? Second, controlling for exporters'
...
Broadly speaking, our paper is about firm dynamics in open economies. As
such, it connects to a number of strands in the literature. One uses
calibrated general equilibrium models with productivity shocks that move
firms through the size distribution. These include Alessandria and Choi
(2007), Ruhl (2007),Alessandria (2010), Atkeson and Burstein (2010),
Burstein and Melitz (2012), Impullitti et al. (2012), Drozd and Nozal
(2012), and Arkolakis (2015). Among these papers, Arkolakis (2015) is
perhaps closest to ours, in that he uses convex market penetration costs to
generate a number of stylized facts, including the age-dependence of export
growth rates.\footnote{%
Drozd and Nozal (2012) also includes a new customer margin and, like us,
treat firms as building their market shares gradually through a costly
search and matching process. However, theirs is a representative agent macro
model that abstracts from firm heterogeneity, endogenous match separations
and learning.} However, since all exporting relationships last a single
period in his models and learning is absent, Arkolakis's models\ do not
explain the irreversibilities observed in firms' exporting behavior, nor do
they speak to the duration of matches. Drozd and Nozal (2012) also include a
new customer margin and, like us, they treat firms as building their market
shares gradually through a costly search and matching process. However,
theirs is a representative agent RBC model that abstracts from firm
heterogeneity. 

More generally, we depart from all of these papers by focussing on firm
growth due to forward-looking investments in their client base rather than
exogenous productivity processes or investments in innovation. Also, unlike
most of this literature, we treat sunk entry costs as incurred at the
buyer-seller match level rather than the foreign border (if at all), and we
allow these investments to pay off in terms of market knowledge and
reputation as well as revenue streams. These latter features of our model
allow us to explain why surviving new exporters tend to rapidly expand, and
why established exporters' sales are relatively stable. They also explain
why many firms export for short periods on a very small scale.

Another strand of the literature using single-agent models to study firms'
customer accumulation in more detail, sacrificing qeneral equilibrium
results to do so. Like ours, these papers rely on customs records. Using
Irish data, Fitzgerald et al. (2019) investigate whether firms build their
customer base through non-price marketing efforts versus price discounts to
establish a market presence. They find little role for discounting, but an
important role for the customer base. Using French data, Pivetau
(forthcoming) also investigates the importance of discounts versus non-price
efforts to build a customer base. Unlike Fitzgerald et al. (2019), he finds
a tendency for firms to discount their goods when they are new to a market,
but he confirms that a large existing customer base improves product
awareness in the remainder of the population and conveys an important
advantage. Our model is perhaps closest to these papers, in that it is also
a single-agent model fit to customs records.\footnote{
[\textit{Tempting to write: They copied us with minimal acknowledgement but
finished more quickly}. \textit{Fitzgerald et al. is especially annoying:
they added learning to their most recent version and acknowledge EEJKT only
as a general "see also" mention in a footnote of their intro.}]} But we
differ from them in that we study the joint evolution of home and foreign
sales for each firm, allow for endogenous entry and exit in both markets,
learning effects, search frictions. Also we characterize the life-cycle of
...
Addional papers with a consumer margin shut down dynamics in order to
analyze assortative matching patterns between exporters and importers in
particular markets. Blum et al. (2010, 2012), Bernard et al. (2018) and
Sugita et al. (2019) explore the buyer-seller matching patterns that emerge
in a full-information world with productivity heterogeneity on both sides of
the market, fixed matching costs, and (in some cases) production
complementarities.\footnote{%
Bernard and Moxnes (2018) provide a useful summary of the literature on
networks and trade.} 

Closer to the present paper is Eaton et al. (2016), which lacks a mechanism
for assortative matching, but treats both buyers and sellers as building
client portfolios by searching for each other in a dynamic market
equilibrium.\footnote{%
An appendix shows how assortative matching can be added.} They, too, find an
important role for the effect of existing business relationships on market
visibility and matching rates. But like the static models, they do not allow
for learning effects, nor do they characterize the life cycle of matches
once they have been formed.

Matching and/or screening frictions appear in a number of other trade models
that are too abstract to bring directly to data, but are supported by
reduced form evidence. In Rauch and Watson (2003), importers experiment with
foreign suppliers by placing trial orders with them, and they gain access to
a supplier network if they establish a successful business relationship. We
take the assumption that importers evaluate a sample shipment before forming
business relationships from this formulation. In Albornoz et al (2012),
firms choose to experiment in markets with low entry costs in order to learn
about their product's appeal elsewhere. Our model resembles theirs in that
exporters learn about their products' through early shipments, and when new
exporters survive, their sales tend to grow rapidly. Aeberhardt et al.
(2014)\ and Araujo et al. (2016) explain the small scale and high exit rate
of new exporters (inter alia) by assuming that exporters are initially
uncertain about the reliability of their new buyers, whom they find through
a random matching process. The learning mechanisms built into these models
are different from ours, but they too are designed to capture some of the
same patterns that we target. \textbf{[JT: this paragraph was based on
superficial readings--I need to confirm that these descriptions are
accurate.]}

Other learning models of export dynamics are designed for structural
estimation. Timoshenko (2015) adds age as a profit shifter to a standard
sunk-cost model of exporter behavior. She then demonstrates that this
variable helps explain exporter persistence in differentiated (but not
homogeneous) product industries and interprets this to imply that learning
effects are important.\ \textbf{[JT: not strictly structural; she used the
Roberts/Tybout 1997 estimator.]} Arkolakis, et al.\ (2018) adapt Jovanovic
(1982) type learning to export market behavior in order to explain the
relatively rapid expansion of new exporters. This paper resembles ours in
that firms learn about their product's appeal in export market by observing
their previous sales. It differs in that it is a calibrated equilibrium
...
important role.\textbf{\ [JT: Mention Dickstein and Morales, 2018?]} %
\renewcommand{\baselinestretch}{1}\footnote{%
...
exchange rate shock over medium-term horizons. Piviteau (forthoming) also
explores these relationships using his model of export dynamics (discussed
above). Our model can be thought of as providing a particular micro
foundation for the dependence of exporting volumes on years in the market.
...
benchmark model fits the data better. \textbf{[Report non-nested test here?]}
We therefore focus our discussion on the results for this model, turning
later to the main distinguishing features of the no-learning results.
...
These figures may seem small, but they are consistent with the data. First,
the productivity distribution for exporting firms come from the right-hand
tail of the unconditional productivity distribution. Thus those firms with
productivity 1.9 standard deviations above the mean unconditional mean of $%
\varphi $ are actually the smaller exporters. Second, since revenues per
shipment are $\eta =5$ times profits per shipment, and since an average of $%
\lambda _{b}=15.43$ shipment occur per year, expected annual revenues from a
match that survives the entire year are $\eta \cdot \lambda _{b}=77.15$
times as large as profits per shipment for that match.
...
this point both histories contain 5 successes.\textbf{\ [JT:\ this doesn't
seem to be true in the graph]}
...
probability of $\theta ^{f}=0.43$. \textbf{[JT: how was this number chosen?]}
...
firms' export revenues ($x$). We conclude our analysis by asking how much
...
peso devaluation on total export trajectories under several alternative
scenarios. These include our benchmark model, the no-learning model, the
model with no network effect ($\gamma =0$), and the model with neither
learning nor network effects. For all simulations, we use the same set of
idiosyncratic and market-wide shocks. \textbf{[JT: I'm not sure I am
describing this correctly. The previous draft said: "we shut down both
foreign market aggregate shocks as well as ideosyncratic buyer shocks"] }
...
\textbf{[JT: maybe figure }\ref{fig:export_dynamics_average}\textbf{\ can go
to an appendix?]}
...
\item Bernard, Andrew, and Andreas Moxnes. 2019. "Networks and Trade." 
