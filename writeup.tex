...
Interestingly, the difference between expansion and contraction elasticities does not derive from differences in match elasticities. These are symmetric in both dimentions, regardless of what time horizon we consider. Rather, it traces mainly to sales per match, implying that selection effects are in play. Specifically, expansion appears to shift the composition of matches toward high productivity firms, which tend to make matches that generate large volumes of sales. And since we know from Section  \ref{sec:margins} that compositional effects do not come from a reallocation of matches between new and incumbent exporters, it must be that positive exchange rate shocks favor high-sales matches more than negative shocks disfavor them. One interpretation is that the smallest, least profitable matches are dropped in a downturn. But in an upturn, there is no selection on idiosyncratic match productivities, since these are unseen until the match is consummated. [\textbf{JT: can we confirm this?}] 
