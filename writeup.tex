...
Finally, to inform our discussion of aggregate export dynamics below, we investigate how the values of firms' exporting operations respond to exchange rate shocks. Specifically, we track transitions in these values over a 25 year period after a 20 percent permanent but unanticipated Colombian peso devaluation, and we repeat the exercise for a permanent but unanticipated 20 percent revaluation. For each experiment we simulate the model 500 times with sequences of randomly-generated aggregate shocks, scaling the value of the random aggregate shocks by 1.20 (for devaluation) or 0.8 (for revaluation) in period 0 and thereafter. The preceeding 25 periods serve as a burn-in.
...
How do these changes in value translate into aggregate export dynamics? We conclude our counterfactual analysis by simulating the aggregate export sales trajectories associated with the exchange rate shocks described above, disaggregating total responses into contributions from (1) buyer-seller relationships that already existed at the time of the shock, (2) new relationships with exporters that were already in the market at the time of shock, and (3) relationships with exporters who were not in the market at the time of the shock.\footnote{%
...
Then, to isolate margins of adjustment, we repeat this excercise for the total number of exporters and the total number of buyer-seller matches.
...
Figure \ref{fig:export_dynamics_macro} summarizes the results. The shaded areas in each panel break down the total value of an export aggregate---sales, matches, or exporters---into contributions from matches that existed in period 0 (yellow), matches created after period 0 with exporters that existed in period 0 (orange), and matches formed after period 0 with exporters that entered after period 0 (blue). Also, each panel includes a set of thin lines that show how the boundaries between the shaded areas would have been different if there had been no permanent shock. Finally, the panels in the left-hand column correspond to a permanent 20 percent peso devaluation and the panels in the right-hand column correspond to a 20 percent peso appreciation, each occuring in period 0. %
\footnote{% 
...
{fig:export_dynamics_macro}, which track the number of active matches. Here the initial valuation effect plays no role, so there is only a small change in the first year after the
...
rate shock is favorable, all types of matches maintain their relative
...
Thus far we have based our analysis on the benchmark version of our model. We now ask how learning effects and market visibility effects shape new cohort dynamics in the aftermath of an exchange rate shock. To isolate the role of learning, we contrast the post-shock evolution pattern implied by our benchmark model (column 1 of table ) with the pattern implied by the known-$\theta ^{f}$ version of the model, normalizing predicted values to unity in the shock period for all aggregates. And to isolate the extra contribution of visibility effects, we use the known-$\theta ^{f}$ version of the model but we give all firms the maximum visibility that firms can achieve upon their market entry. \footnote{%
Since the visibility effect maxes out at $m = 40$, this amounts to repacing equation (\ref{cost of effort}) with \begin{equation}
c^{m}(s^{m},a^{m})=\kappa _{0}^{m}\frac{\left[ (1+s^{m})\right] ^{\kappa
_{1}}-1}{\kappa _{1}\left[ 1+\ln (1+a^{40})\right] ^{\gamma }}.
\end{equation}.}

Figure \ref{fig:export_dynamics_average} depicts the simulated responses of total sales, total matches, and number of exporters to a 20 percent exchange rate appreciation (left-hand panels) and a 20 percent exchange rate depeciation (right-hand panels). As in figure \ref{fig:export_dynamics_macro}, each panel distinguishes the contributions of firms that were incumbent exporters at the time of the shock (yellow region), exporters that entered after the shock (orange region) and post entry increases in the number of matches of these new exporters (blue region). But in this figure, we have superimposed lines that show how the shaded areas would have shifted if firms knew the $\theta ^{f}$ values with certainty (dashed red lines), and if they not only knew their $\theta ^{f}$ values, but also had "full" visibility (dashed green lines).
...
Several clear messages emerge from this exercise. First, regardless of whether learning effects and visibility effects are present, entering cohorts displace pre-shock incumbents at almost exactly the same pace. That is, the borders between the orange and yellow regions nearly coincide with the dashed red and green lines in all panels. Second, however, if firms had known their true $\theta^{f}$ values before acquiring foreign market experience, substantially more displacement would have been due to new exporters, and substantially less to the addition of new clients by new exporters.  Third, visibility effects matter a bit, especially for total active matches, but they actually reduce the share of new exporters relative to the case where $\theta^{f}$ draws are known with certainty but visibility must be gradually built through successful matches. Finally, none of these patterns depends upon whether we are considering a permanent appreciation or a permanent depreciation--they are general features new cohort maturation processes.
