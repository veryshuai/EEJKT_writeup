...
vector of simulated moments. \textquotedblleft Intuitively, this matrix is a local
approximation to the mapping from moments to estimated parameters.\textquotedblright
...
firms with productivity 1.9 standard deviations above the unconditional
...
How well does the model replicate the patterns in Section \ref{sec:data}? Direct comparisons to these data are problematic, since the results in  in Section \ref{sec:data} follow the literature in presuming that any transaction between a buyer and a seller constitutes a match, but our parameter estimates presume that successful matches generate at least two shipments.\footnote{%
Restrictions on data access have thus far prevented us from re-doing
these tables in a fully compatible way.} Nonetheless, we note that the stylized facts in Section \ref{sec:data} are qualitatively replicated by
our model. Tables \ref{tab:brooks_compare}, %
\ref{tab:client_dist_compare} and \ref{tab:match_exit_compare} below provide details. Here, as in Section \ref{sec:data}, the values of each aggregate for
