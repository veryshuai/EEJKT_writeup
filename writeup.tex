...
In recent years, researchers have directed considerable attention at the question of what drives firm-level export dynamics. Various models have explored the roles of market entry costs, learning, search frictions, and reputation-building. We take this analysis one step forward by developing a dynamic model that simultaneously incorporates all of these elements and quantifies their individual effects on exporters' behavior. Further, we do so by focussing on the formation and maturation of exporters' relationships with individual foreign buyers. In doing so we quantify the exporting costs faced by different types of firms, as well as the value of the intangible capital they have amassed by building their portfolios of foreign clients. Finally we perform several experiments that quantify the implications of our model's features  for aggregate export dynamics.

Our model incorporates and separately quantifies three types
...
$ \$US $45 to \$US 405\textbf{.} Further, a firm with the highest
...
In Figure \ref{fig:avg_value} we plot the average and median value of firms'
intangible export-related capital during pre- and post-shock periods.\footnote{%
Both the means (panel a) and the medians (panel b) are averages across 500 simulations.} At each point in time, these series indicate how much a typical exporter would lose if we returned it to the state it was in before it began meeting foreign clients. Thus they represent the value of the information it has learned about its success rate $\theta ^{f}$, plus the value of the visibility it has built through previous matches, plues the expected profits yet to be generated by its current portfolio of cleints. Since they were calculated under the presumption that firms are free to re-enter export markets after their exporting histories are wiped out, they substantially understate the capital losses that would result from a permanent move to autarky.
...
The value of experience is on average around 18 percent of firms' total (export-related) intangible capital, both before and after the exchange rate shocks. For a back-of-the-envelope calculation of the total value of exporting experience in the economy, we multiply 1.7 million dollars by the approximately 3000 Colombian exporters we observe each year. We get that the total value of exporter experience in the Colombian economy is about 5.1 billion dollars. 
...
plays no role, so there is only a small change in the first year after the
...
relative to incumbents. With the unfavorable shock, both groups immediately
...
there is information content in each match they make, and being smaller,
they are operating in a less convex portion of the search cost function.
Therefore, compared to the benchmark, no-shock scenario, new exporters gain
market share as the importance of relationhips formed in the post-shock
period grows over time (Panels b and d of figure \ref%
{fig:export_dynamics_macro}).\textbf{\ [JT: this explanation may not be
correct.]}
...
mechanical change in matches from the exchange rate shock.
