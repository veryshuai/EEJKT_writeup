...
Finally, to inform our discussion of aggregate export dynamics below, we characterize the responses of firms' values to permanent but unanticipated switches in the exchange rate regime. Specifically, we generate 500 sequences of 50 foreign demand realizations using the process reported in Table \ref{tab:dem_shift}. Then we scale all realizations after the $25^{th}$ period by 1.20 (for our devaluation experiment) or 0.8 (for our revaluation experiment), and for each trajectory, we simulate all firms' exporting patterns using our estimated model. 
...
During the transition period, all margins of adjustment move in sympathy. In fact,  if we were only interested in the rate at which new exporters displace incumbent exporters, or the rate at which new matches displace old matches, it would matter very little whether we were analyzing the aftermath of a permanant devaluation, a permanent revaluation, or a period without any regime swithing. This is apparent from Figure \ref%
{fig:export_dynamics_average}, which plots the percentage contribution of each type of export to exports aggregates. Note that the post shock transition patterns in all figures coincide very closely with the no-shock (thin line) patterns. Thus in our benchmark model, the relative contributions of matches at new versus existing exporters to aggregate export dynamics are not disturbed by aggregate shocks.


\subsubsection{Learning, networks and aggregate export dynamics}
Nonetheless, these relative contributions \emph{do} depend upon the learning effects and visibility effects we have embedded in our model. We now demonstrate exactly how. To isolate the role of learning, we contrast the post-shock evolution pattern implied by our benchmark model (column 1 of table ) with the pattern implied by the known-$\theta ^{f}$ version of the model, normalizing predicted values to unity in the shock period for all aggregates. And to isolate the extra contribution of visibility effects, we use the known-$\theta ^{f}$ version of the model but we give all firms the maximum visibility that firms can achieve upon their market entry. \footnote{%
...
Several clear messages emerge from this exercise. First, regardless of whether learning effects and visibility effects are present, entering cohorts displace pre-shock matches at almost exactly the same pace. That is, the borders between the orange and yellow regions nearly coincide with the dashed red and green lines in all panels. Second, however, if firms had known their true $\theta^{f}$ values before acquiring foreign market experience, substantially more displacement would have been due to new exporters, and substantially less to the addition of new clients by pre-shock exporters.  Why? When potential exporters know their product appeal, only the ones with high $\theta^{f}$'s enter. And when they do, they immediately search intensively, knowing their probability of success on any given match is relatively high. 
Conditioning on full knowledge of $\theta^{f}$, full visibility matters a bit, especially for total active matches. It shifts the share of total matches toward pre-shock entrants and away from post-shock entrants relative to the case where $\theta^{f}$ draws are known with certainty but visibility must be gradually built through successful matches.This effect becomes more important over time as the share of post-shock entrants in the population of exporters grows (refer to panels e and f).
 Finally, none of these patterns depends upon whether we are considering a permanent appreciation or a permanent depreciation--they are general features new cohort maturation processes.
