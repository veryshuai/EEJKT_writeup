...
\begin{abstract}
BLAH Customs record data reveal a number of patterns in relationships Colombian
...
Column 1 of Table \ref{tab:Brooks} shows the
...
Column 2 shows that the rapid initial decline in its membership is
...
U.S. importing firms grew by 3 percent per annum (Appendix \ref{sec:data_tables}, Table \ref%
...
English.  Interviewees also reported that relatively low-cost
...
expected success rate on search histories$.$  Again suppressing market superscripts, the
...
of firms' exporting behavior: match termination policies. Here the
...
\footnote{%
Note, however, that in Table \ref{tab:client_dist}, matches that die after a single shipment are
...
Not surprisingly, estimates of equation ($ii$) reflect the same patterns that
we noted in connection with Table \ref{tab:client_dist}. Matches in their first year are
...
Colombian customs records.\footnote{%
    More precisely, regressions ($viii$) through ($x$) in Table \ref{tab:hf_sales} are done using a
...
discussed), we obtain the results reported in detail in Appendix \ref{sec:sensitivity}. Here we summarize the patterns that emerge.
...
benchmark model fits the data better. \textbf{[Report non-nested test here?]} We
...
These figures may seem small, but they are consistent
...
Appendix \ref{sec:model_fit} juxtaposes the data-based moments, $\bar{m},$ with their
...
data. A more detailed summary of the fit can be found in Appendix \ref{sec:model_fit}.

Since we have not targeted the patterns described in Section \ref{sec:data} when
estimating, it is instructive to check how well they are
...
in Tables \ref{tab:Brooks}-\ref{tab:erg_cli_dist} is collapsed by averaging across exporting cohorts for which
...
\begin{tablenotes}
%\bigskip
\item \textbf{Notes:} Figrues are percentages of the exporters in each age-initial size category that do not export during the following year.
\end{tablenotes}
}
\end{table}
