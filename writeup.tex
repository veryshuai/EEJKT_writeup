...
%TCIDATA{LastRevised=Wednesday, August 12, 2020 13:46:14}
...
state earns \$US\textbf{\ }31,512 in gross profits per export shipment and 
\textbf{\$}US\textbf{\ }281,570 in profit per domestic shipment. Of course,
...
\subsubsection{Comparison with known-$\protect\theta ^{f}$}
...
\textbf{The value of connections }In Figure \ref{fig:avg_value} we plot the
average value of simulated firms net of their value had they not had any
export experience. That is, suppose that a firm has met a number of clients,
and has both learned something about its success rate $\theta ^{f}$. In
addition, this firm has accumulated network effects through its history. We
take the firm's value, and subtract from it the value of an otherwise
similar firm, but with no export experience. We call this object the value
of the firm's export experience. It is the value a firm would lose if we
made it forget everything it learned and gained from exporting, including
its current portfolio of cleints.
...
\textbf{Margins of adjustment} Figures \ref{fig:export_dynamics_macro},
inspired by the decomposition plots in Piveteau (2020), summarize the
macroeconomic effects of our exchange rate experiments. They break down the
total value of exports into the contribution from (1) buyer-seller
relationships that existed at or before the time of the shock (yellow), (2)
new relationships with exporters that were already in the market at the time
of shock (orange), and relationships with exporters that were not in the
market at the time of the shock (blue). 

Relative to Piveteau's (2020) model, our formulation includes several
features that contribute to asymmetric exchange rate responses. First, we
allow for endogenous match separations and search effort. This means that
negative exchange rate shocks cause marginally profitable matches to
immediately terminate, while positive exchange rate shocks induce a gradual
accumulation of client connections. Second, we allow exporters to learn
about their product appeal through each new match they make. Thus, by
increasing search intensities, positive exchange rate shocks lead to
accelerated learning and selection effects for inexperienced exporters,
which are relatively common during an expansion. But negative exchange rate
shocks do not destroy any of the knowledge already accumulated by incumbent
exporters. Rather, they slow learning processes for the relatively scarce
new exporters that appear during a contraction. To highlight these features
of our model, we use a decompostion that distinguishes matches to new
exporters from others.\footnote{%
...
which plot total export sales over a 50 year period bisected by the shock.
The superimposed lines show the decomposition in the situation in which
there was no re-scaling of the exchange rate process, and thus characterize
the evolution of a typical cohort in an ergodic environment. Note that
...
when the exchange rate depreciates (appreciates).

A similar pattern can be seen in Panels (c) and (d) of Figure \ref%
{fig:export_dynamics_macro} which track matches. Here the valuation effect
plays no role, so there is only a small change the first year after the
...
{fig:export_dynamics_average}. Summarizing the figures, when the exchange
rate shock is favorable, all margins maintain their relative importance, but
with an unfavorable shock, new exporters gain market share relative to
incumbents. Both groups immediately terminate their marginal matches,
cleansing the market of low-profitability relationships (Panel d, Figure \ref%
{fig:export_dynamics_macro}).\footnote{%
...
are less inclined than experienced exporters to cut back on search, since
there is information content in each match they make. Therefore, compared to
the benchmark, no-shock scenario, new exporters gain market share as the
importance of relationhips formed in the post-shock period grows over time
(Panels b and d of figure \ref{fig:export_dynamics_macro}).

\textbf{Trade elasticities} Finally, we present short and long-run simulated
trade elasticities in Table \ref{tab:elasticities}. Based on the point
estimates alone, total sales react more to the favorable exchange rate
shock. The 25 year elasticity of sales to the favorable exchange rate shock
is 2.22.\footnote{%
The trade elasticities in our model for the favorable exchange rate shock
are a bit more sluggish than those found in a analogous experiment by
Piveteau (2020). His point estimate for the one year trade elasticity is
1.3, and his estimate for the five year trade elasticity is 2.3.} The 25
year elasticity of sales to the unfavorable exchange rate shock is 1.62. We
can reject the hypothesis that these elasticities are the same. We find
evidence that sales adjustment to favorable exchange rate shocks is larger
than to unfavorable exchange rate shocks. This asymmetry reflects . . .

These elasticities are smaller, and the transition period is longer, than
those reported by Piveteau (2020), Alessandria and Choi (2014), and
Alessandria, et al. (2018).\footnote{%
Piveteau (2020) also uses a single agent model with customer accumulation,
but with no learning about product appeal and no marketing or search
investements. He estimates a long-run elasticity of roughly 2.3, with a
transition period spanning 5 years. Alessandria and Choi (2014) use a
symmetric 2-country dynamic model with endogenous firm creation, capital
accumulation, fixed exporting costs, and iceberg costs but no other trade
frictions or factor adjustment costs. Analyzing movement from a global 8
percent tariff to free trade, they find the trade elasticity rises from
about 5 in the short run to 8 in the long run, which is reached in 5-8
years. In a similar model, but with firms' exporting costs depending upon
their incumbency, Alessandria et al. (2018) estimate a short-run trade
elasticity of 4 and a long-run elasticity of 11.55. This model generates
transition dynamics over a period of 10-15 years.}\textbf{\ }The larger
elasticities that obtain in the latter two papers are partly due to their
general equilibrium\ . . .  The longer transition period in our paper
reflect the large role our model assigns to search frictions and learning.

The elasticities for matches in Table \ref{tab:elasticities} are around one
unit smaller than those for sales, since there is no mechanical change in
matches from the exchange rate shock. Here the values for favorable and
unfavorable shocks are similar, although the point estimates are a bit
smaller for unfavorable shocks. The point estimates for firms have the
opposite pattern. They are larger for the unfavorable exchange rate shock
than for the favorable shock, particularly after 25 years. This difference
...
\begin{tabular}{llll}
\hline\hline
& \multicolumn{3}{l}{\textbf{Favorable elasticities}} \\ \cline{2-4}
...
& (0.03) & (0.03) & (0.03) \\ 
& \multicolumn{3}{l}{\textbf{Unfavorable elasticities}} \\ \cline{2-4}
Time since shock & 1 year & 5 years & 25 years \\ \hline
