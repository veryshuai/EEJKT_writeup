...
Thus far we have based our analysis on the benchmark version of our model. We now ask how learning effects and market visibility effects contribute to the process through which new cohorts mature in response to an exchange rate shock. To isolate the role of learning, we contrast the post-shock evolution pattern implied by our benchmark model (column 1 of table ) with the pattern implied by the known-$\theta ^{f}$ version of the model, normalizing preditcted to unity in the shock period for all aggregates. And to isolate the extra contribution of visibility effects, we use the known-$\theta ^{f}$ version of the model but we give all firms the maximum visibility that firms can achieve upon their market entry. 
